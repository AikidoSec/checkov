...
def test_create_cli_cves_table(mocker):
    ...
    # then (prettytable 3.16.0: 6 cols, 126 width → 19-char columns)
    assert table == "".join(
        [
...
def test_create_cli_license_violations_table(mocker):
    ...
    # then (prettytable 3.16.0: 5 cols, 126 width → 24/25-char columns)
    assert table == "".join(
        [
...
def test_create_cli_cves_table_with_no_found_vulnerabilities(mocker):
    ...
    # then (prettytable 3.16.0: header=False → no cell padding; fixable table width 127)
    assert table == "".join(
        [
            "\t/path/to/requirements.txt - CVEs Summary:\n",
            "\t┌───────────────────┬───────────────────┬───────────────────┬───────────────────┬───────────────────┬───────────────────┐\n",
